...
const { POSITION_MANAGER_ADDRESS, DECIMALS_18, etherBalances, rates, durations, ONE_WEEK_IN_SECONDS, MOST_STABLE_FEE, STABLE_TICK_SPACING, encodePriceSqrt, helperFastForwardTime, MAX_TICK, MIN_TICK, scaleUpForDecDiff, CHAINLINK_DEC, DEFAULT_CHAINLINK_EUR_USD_PRICE, getLibraryFactory, eurToTST, defaultConvertUsdToEur } = require('../common.js');
...
describe('BondingEvent', async () => {
  ...
  const deployBondingEventWithDefaultPrices = async () => {
    // USDC / sEURO price of 1.10 isn't really reflective of the starting price for bonding event
    // but it puts it nicely between our default ticks
  ...
  context('initialised with default prices', async () => {
    ...
    describe('calculating ratio', async () => {
      it('calculates the required amount of USDC for given sEURO', async () => {
        ...
        amountOther = amountOther.div(DECIMALS_18);
        // comes from uniswap ui
        const expectedUSDC = 8172;
    ...
    describe('bond', async () => {
      ...
      it('bonds multiple times with various maturities and updates active and inactive bonds correctly', async () => {
        ...
        // 5%
        const seuroProfit1 = amountSeuro.div(20);
        const usdcProfit1 = amountOther.div(20);
        let expectedProfit1 = eurToTST(seuroProfit1).add(eurToTST(defaultConvertUsdToEur(usdcProfit1)));
        expect(await helperGetProfit()).to.eq(expectedProfit1);
        ...
        // 5% + 6%
        const seuroProfit2 = amountSeuro.mul(6).div(100);
        const usdcProfit2 = amountOther.mul(6).div(100);
        expectedProfit2 = eurToTST(seuroProfit2).add(eurToTST(defaultConvertUsdToEur(usdcProfit2)));
        expect(await helperGetProfit()).to.eq(expectedProfit1.add(expectedProfit2));
        ...
        // 5% + 6% + 10%
        const seuroProfit3 = amountSeuro.div(10);
  ...
  describe('liquidity positions', async () => {
    context('initialised default prices', async () => {
      ...
      it('creates a second position if the price moves after initialisation', async () => {
        ...
        // moves current price tick to 1412
  ...
  describe('liquidity ratios', async () => {
    ...
    it('gives the default if current price in middle 20% between ticks', async () => {
      // puts price tick at 953, between 40th + 60th percentile between -1000 and 2700 ticks
      await deployBondingEventWithDefaultPrices();
      await mintUsers();
      await readyDependencies();
      ...
      // initial current price tick is at 953, we move it towards 2700 gradually (via swapping the seuro for the usdt liquidity in there)
      // swaps 200k sEURO for USDT and the tick stays at 1138, which is within middle 20% of -1000 and 2700 ticks
      await swap(SEuro, USDC, etherBalances['100K'].mul(2));
    ...
    it('gives a new tick range if current price outside of range of default ticks', async () => {
      // sets current price tick to 3001, outside of ticks -1000 and 2700
      await deployBondingEvent(135, 100);
      const amountSEuro = etherBalances.TWO_MILLION;

      const { lowerTick, upperTick } = await BondingEvent.getOtherAmount(amountSEuro);

      // 9000 expansion to -10000 and 11700 satisfies 40th - 60th percentile buffer
    ...
    it('gives a new tick range if current price close to edge of tick range', async () => {
      // sets current price tick to 2231, outside of middle 20% of -1000 and 2700
      await deployBondingEvent(125, 100);
      const amountSEuro = etherBalances.TWO_MILLION;

      const { lowerTick, upperTick } = await BondingEvent.getOtherAmount(amountSEuro);

      // 6000 expansion to -7000 and 8700 satisfies 40th - 60th percentile buffer
    ...
    it('gives max limit tick range if current price is very extreme', async () => {
      // sets current price tick to -276325
